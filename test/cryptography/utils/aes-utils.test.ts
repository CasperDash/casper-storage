...
test("aes-utils.encrypt-invalid-key", async () => {
  await expect(AESUtils.encrypt(null, null, null, null)).rejects.toThrowError("Key is required");
})

test("aes-utils.encrypt-invalid-value-null", async () => {
  await expect(AESUtils.encrypt(TEST_PWD_01, null, null, null)).rejects.toThrowError("Value is required");
})

test("aes-utils.encrypt-invalid-salt", async () => {
  await expect(AESUtils.encrypt(TEST_PWD_01, "Test", null, null)).rejects.toThrowError("Salt is required");
})

test("aes-utils.encrypt-invalid-iv", async () => {
  await expect(AESUtils.encrypt(TEST_PWD_01, "Test", CryptoUtils.randomBytes(5), null)).rejects.toThrowError("IV is required");
})
...
test("aes-utils.decrypt-invalid-salt", async () => {
  await expect(AESUtils.decrypt(TEST_PWD_01, "Test", null, null)).rejects.toThrowError("Salt is required");
})

test("aes-utils.decrypt-invalid-iv", async () => {
  await expect(AESUtils.decrypt(TEST_PWD_01, "Test", CryptoUtils.randomBytes(5), null)).rejects.toThrowError("IV is required");
})

test("aes-utils.encrypt-decrypt-simple_value", async () => {
  const salt = CryptoUtils.randomBytes(16);
  const encryptedText = await AESUtils.encrypt(TEST_PWD_01, "Simple test value", salt, salt);
  const decryptedValue = await AESUtils.decrypt(TEST_PWD_01, encryptedText, salt, salt);
  expect(decryptedValue).toBe("Simple test value");
})

test("aes-utils.encrypt-decrypt-char", async () => {
  const salt = CryptoUtils.randomBytes(16);
  const encryptedText = await AESUtils.encrypt(TEST_PWD_01, "a", salt, salt);
  const decryptedValue = await AESUtils.decrypt(TEST_PWD_01, encryptedText, salt, salt);
...
test("aes-utils.encrypt-decrypt-long_value",async () => {
  const salt = CryptoUtils.randomBytes(16);
  const encryptedText = await AESUtils.encrypt(TEST_PWD_01, "aaaaaaaaaaaaabbbbbbbbbbbbccccccccccccccddddddddddddddeeeeeeeeeeeeefffffffffffffff", salt, salt);
  const decryptedValue = await AESUtils.decrypt(TEST_PWD_01, encryptedText, salt, salt);
...
test("aes-utils.encrypt-decrypt-super_long_value", async () => {
  ...
  const encryptedText = await AESUtils.encrypt(TEST_PWD_01, text, salt, salt);
  const decryptedValue = await AESUtils.decrypt(TEST_PWD_01, encryptedText, salt, salt);
