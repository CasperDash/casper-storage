...
test("coint-path.ctor.bip44-bitcoin", () => {
  let coinPath = new CoinPath(DEFAULT_COINT_PATH, Purpose.BIP44, CoinType.Bitcoin);
  expect(coinPath.path).toBe("m/44'/0'/INDEX'");
});

test("coint-path.ctor.bip44-bitcoin-testnet", () => {
  let coinPath = new CoinPath(DEFAULT_COINT_PATH, Purpose.BIP44, CoinType.BitcoinTestnet);
  expect(coinPath.path).toBe("m/44'/1'/INDEX'");
});

test("coint-path.ctor.bip44-casper", () => {
  let coinPath = new CoinPath(DEFAULT_COINT_PATH, Purpose.BIP44, CoinType.Casper);
  expect(coinPath.path).toBe("m/44'/506'/INDEX'");
});

test("coint-path.ctor.bip44-casper", () => {
  let coinPath = new CoinPath(DEFAULT_COINT_PATH, Purpose.BIP49, CoinType.Casper);
