...
test(("casper-wallet-utils.ed25519"), async () => {
  let value = CasperWalletUtils.getAccountAddress(EncryptionType.Ed25519, "testkey");
  expect(value).toBe("01testkey");
});

test(("casper-wallet-utils.secp256k1"), async () => {
  let value = CasperWalletUtils.getAccountAddress(EncryptionType.Secp256k1, "testkey");
